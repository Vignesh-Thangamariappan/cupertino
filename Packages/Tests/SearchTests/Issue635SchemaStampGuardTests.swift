...
struct Issue635SchemaStampGuardTests {
    ...
    func matchingVersionIsNoOp() async throws {
        ...
        let idx1 = try await Search.Index(dbPath: dbPath, logger: Logging.NoopRecording(), indexers: [:], sourceLookup: .empty)
        ...
        let idx2 = try await Search.Index(dbPath: dbPath, logger: Logging.NoopRecording(), indexers: [:], sourceLookup: .empty)
    ...
    func schemaVersionMinusOneAutoMigratesToTarget() async throws {
        ...
        let reopened = try await Search.Index(dbPath: dbPath, logger: Logging.NoopRecording(), indexers: [:], sourceLookup: .empty)
