...
The output of stage 1 is consumed only by the TUI's popularity-sort view. The indexing pipeline (`packages.db`) does not need it. So as of #1108 stage 1 is opt-in via this flag; `cupertino fetch --source packages` runs stage 2 only by default. Pass `--refresh-metadata` when you actually want to refresh the stars list.
