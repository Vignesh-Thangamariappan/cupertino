...
In fan-out mode (no `--source`), each contributing fetcher (apple-docs, samples, packages, etc.) returns up to `--per-source` candidates before they're cross-ranked via RRF. Caps a noisy source from drowning out a strong single hit from another. (#239)
