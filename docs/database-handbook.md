...
> **Physical layout as of v1.4.0 (2026-06-21):** the unified `search.db` this handbook and `docs/architecture/database.md` describe is shipped as 8 per-source databases (`apple-documentation.db`, `hig.db`, `apple-archive.db`, `swift-evolution.db`, `swift-org.db`, `swift-book.db`, `apple-sample-code.db`, `packages.db`), each in rollback journal mode so it opens read-only without an `-shm` sidecar. Every query / read / serve connection opens the databases read-only (#1194), so an end user cannot write or delete rows. The split (introduced in v1.3.0) is physical packaging only: the per-database schema, BM25F weights, enrichment passes, and ranking are unchanged, so the `search.db` schema reference in `docs/architecture/database.md` and the v1.2.0 search-quality baselines in §5 still apply per database. v1.4.0 is a content refresh on the same layout (re-crawled corpus, `apple-documentation.db` 363,562 docs / 308,118 symbols with iOS 27 content, placeholder-stub rot removed per `scripts/check-db-quality.sh`). `cupertino setup` downloads `cupertino-databases-v1.4.0.zip` (876 MB).
