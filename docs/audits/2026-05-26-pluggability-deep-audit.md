...
- [x] **9.7 + 11.1** — `cupertino fetch` dispatch + per-source crawler concretes. Phase 1 lifted 6 of 9 handlers behind a new `Search.SourceProvider.makeFetchStrategy()` protocol method; Phase 2 (CLOSE) lifted the remaining 3 complex handlers (`runSamplesFetch` → `SampleCodeFetchStrategy`; `runPackageFetch` 3-stage pipeline → `PackagesFetchStrategy`; the apple-sample-code legacy alias canonicalizes to `samples` at dispatch). Physically MOVED the per-source `Crawler.<X>` concretes (HIG / Evolution / AppleArchive / AppleDocs + state files + TechnologiesIndex + ArchiveGuideCatalog) into their owning source targets via `git mv`; the now-empty `Crawler` and `Ingest` SPM targets + their test targets + the `.singleTargetLibrary("Ingest")` product all deleted. `Ingest.Session` + `Ingest.swift` (session resume / requeue / baseline / urls-file helpers) lifted into `AppleDocsSource`. The `Fetch.swift` switch collapsed to 2 special-token arms + `default → runRegistryFetchStrategy`; the 22 dead methods + 8 dead progress observer structs (~1500 LOC) deleted. Adding a new source = ONE `Search.SourceFetchStrategy` concrete in its target + `.register(<X>Source())` in `Cupertino.CompositionRoot.swift`. Zero edits to `Fetch.swift`, `Crawler` (the producer target is gone), or any cross-cutting CLI plumbing.

## Layer-2 follow-up — fractal anti-pattern at deeper layers

After Layer-1 closure the user demanded "while on autopilot, go as deep as possible". Three further per-source edit-points surfaced and closed:

- [x] **#1055 layer-2 part 1**: `Services.ReadService.read` was still bucket-dispatching `if source == .docs/.samples/.packages` to one of 3 backend handlers — a hardcoded 3-arm switch. Lifted to `Search.SourceProvider.makeReadStrategy()` protocol method returning a `Search.SourceReadStrategy` concrete. 6 docs-tier sources return a shared `Search.DocsReadStrategy` parameterised on source-id; `SampleCodeSource` returns `SamplesReadStrategy`; `PackagesSource` returns `PackagesReadStrategy`. `Services.ReadService.read` iterates `providers: [any Search.SourceProvider]` and runs the first strategy that returns non-nil. The 3 lookup protocols (`Search.DocsLookupStrategy` / `SampleLookupStrategy` / `PackageFileLookupStrategy`) live in SearchModels foundation tier. Root-cause of the 2 test failures during WIP: `makeReadStrategy` was declared only as a default extension, not as a protocol requirement — Swift's static dispatch resolved through the default (returning nil) instead of per-source overrides; fix: hoist into the protocol body.

- [x] **#1055 layer-2 part 2**: `Search.SearchRoute` was a closed enum with 5 cases. Both `CLIImpl.Command.Search.run` and `SearchToolProvider.CompositeToolProvider.handleSearch` had `switch route` arms — a new route required a new enum case AND matching arms in BOTH dispatchers. Lifted to a `RawRepresentable` struct mirroring the `Search.FetchInfo.DefaultOutputDirKey` Cluster-9-sub-1 shape; dispatchers use `if route == .X` and fall through to `.unified` on any unrecognised route. The 3 source-id-shaped routes read from `Shared.Constants.SourcePrefix.*`; the 2 bucket-tier routes get canonical names in a new `Shared.Constants.SearchRouteName` namespace.

- [x] **#1055 layer-2 part 3**: `CLIImpl.Command.Search.SmartReport.docsSources()` filtered the registry by a hardcoded `excluded: Set<DatabaseDescriptor> = [.appleSampleCode, .packages]`. Any new source with a non-FTS backend had to be appended to that set in the same PR. Lifted to `var isSearchTier: Bool { get }` protocol requirement on `Search.SourceProvider` with `true` default; `SampleCodeSource` and `PackagesSource` declare `false`. Filter becomes `.filter(\.isSearchTier)`; the hardcoded descriptor set is gone.

Finding 10.1 (`buildFetchers` 3-bucket openers) stayed at "mild" — the 3-arm split corresponds to 3 DB backend technologies (search.db FTS / packages.db BM25 / apple-sample-code.db catalog), not 3 sources. `openDocsFetchers` is registry-aware; a new source within the FTS family plugs in via the `isSearchTier`-filtered `docsSources()` iteration with zero edits to SmartReport.

**Final state**: every finding in this audit's ledger closed plus 3 layer-2 deepenings. A new source within the FTS family is a 2-file PR (`Packages/Sources/<X>Source/<X>Source.swift` + one `.register(<X>Source())` line in `Cupertino.CompositionRoot.swift`). The HOW-TO-ADD-A-SOURCE.md doc needs a rewrite to reflect this (queued as Issue F).
